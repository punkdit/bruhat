...
#        return c1
    ...
    def unify(self, c1, c2):
        ...
        to_unify = [(c1, c2)]
        while to_unify:
            assert neighbors is self.neighbors
            c1, c2 = to_unify.pop()
            c1 = self.get_label(c1)
            c2 = self.get_label(c2)
            if c1 == c2:
    ...
    def add_vertex(self): # may change neighbors object
        ...
        if self.idx == n:
            ...
            neighbors = cpy
            self.neighbors = neighbors
        neighbors[self.idx, :] = -1
        self.idx += 1
        assert self.idx < 2**31, 'wup'
        return c

    # For following paths, we have the following two functions: 
    
    def follow_step(self, c, d): # may change neighbors object
        labels = self.labels
        neighbors = self.neighbors
        c = self.get_label(c)
        ns = neighbors[c]
    ...
    def follow_path(self, idx, word): # may change neighbors object
    ...
    def build(self, hgens=[], maxsize=None):
        ...
        while to_visit < len(labels):
            ...
            if maxsize and self.idx > maxsize:
                return None
        return self
