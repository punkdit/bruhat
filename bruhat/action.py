...
from bruhat.util import factorial, all_subsets, write, uniqtuples, cross, disjoint_union, all_functions
...
def mulclose_find(gen, names, tgt, verbose=False, maxsize=None):
    bdy = list(set(gen))
    assert len(names) == len(gen)
    names = dict((gen[i], (names[i],)) for i in range(len(gen)))
    if tgt in gen:
...
def mulclose_hom(gen1, gen2, verbose=False, maxsize=None):
    "build a group hom from generators: gen1 -> gen2. Bail out if there is none."
    hom = {}
