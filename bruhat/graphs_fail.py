...
def coxeter_BC3():
    ...
    G = Group.coxeter_bc(3, check=True)
    ...
    parents = {g:[] for g in G}
    found = {G.identity}
    edges = []
    bdy = list(found)
    waves = []
    while bdy:
        waves.append([lookup[g] for g in bdy])
        _bdy = []
        for g in G.gen:
            for h in bdy:
                gh = g*h # left cayley graph
                edges.append( (lookup[g], lookup[gh]) )
                if gh in found:
                    continue
                parents[gh].append(g)
                found.add(gh)
    ...
    print("yield!")
    return [ make_waves(nodes, edges, waves) ]


def make_waves(nodes, edges, waves):
#    print(nodes)
#    print(edges)
#    print(waves)
#    print([len(l) for l in waves])
    assert sum([len(l) for l in waves]) == len(nodes)
...
def main():
    ...
    if n is not None:
        ...
        items = fn()
        assert items is not None, "%s returned None"%(graph_name,)
